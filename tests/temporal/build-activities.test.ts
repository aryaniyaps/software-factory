...
describe("build activities", () => {
  ...
  it("rejects mutable or malformed artifact references", async () => {
    ...
    await expect(activities.buildArtifact({ run: { runId: "run", taskId: "task", repository: "/repo", baseBranch: "main", workflow: "feature", deploymentProfile: "staging", sandboxProfile: "crabbox" }, worktree: { path: "/worktree", branch: "factory/run/task/1" } })).rejects.toThrow("immutable image digest");
  });
});
