...
describe("retry policy", () => {
  ...
  it("returns abstain when budget is exhausted", () => {
    const exhausted = {
      ...DEFAULT_WORKFLOW_BUDGET,
  ...
  it("retries transient failures until max attempts then abstains", () => {
    ...
    expect(decideAfterFailure(transient, 3, 3, DEFAULT_WORKFLOW_BUDGET)).toBe("abstain");
  });
