...
describe("HindsightMemory", () => {
  it("uses the scoped bank and correlation document id", async () => {
    ...
    const recall = vi.fn().mockResolvedValue({ results: [{ text: "remembered" }] });
    const reflect = vi.fn().mockResolvedValue({ text: "insight" });
    const memory = new HindsightMemory({ retainBatch, recall, reflect } as never);
    ...
    expect(retainBatch).toHaveBeenCalledWith(
      "project:demo",
      [{ content: "Use strict TypeScript", document_id: "run-1:ticket-1:attempt-1:scout", tags: ["run:run-1", "phase:scout"] }],
      { async: true },
    );
