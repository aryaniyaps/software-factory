...
describe("ProjectReconciler", () => {
  it("starts one Temporal workflow per ready project item", async () => {
    ...
    const reconciler = new ProjectReconciler(
      { listReady: async () => [{ id: "item-1", projectId: "project", projectItemId: "item-1", title: "A", description: "A", repository: "org/a", baseBranch: "main", workflow: "feature", deploymentProfile: "staging", sandboxProfile: "crabbox" }], updateStatus: async (_id, status) => { statuses.push(status); } },
